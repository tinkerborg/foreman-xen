  ...
  class Xenserver < ComputeResource
    ...
    def host_interfaces_attrs(host)
      host.interfaces.select(&:physical?).each.with_index.reduce({}) do |hash, (nic, index)|
           hash.merge(index.to_s => nic.compute_attributes.merge(ip: nic.ip, ip6: nic.ip6, subnet: "#{nic.subnet.network}", netmask: "#{nic.subnet.mask}", vlan: "#{nic.subnet.vlanid}"))
      end
    end

    def create_network(vm, args)
      vifs = []
      args[:interfaces_attributes].each do |index, interface|
        net = client.networks.find do |network|
          network.pifs.any? do |pif|
            if interface[:vlan] == pif.vlan
              true
            elsif pif.vlan.to_i == -1 && ! pif.ip.empty?
              IPAddr.new("#{interface[:subnet]}/#{interface[:netmask]}").include?(pif.ip)
            end
          end
        end
        ...
        if net
          ...
          vifs << client.vifs.create(net_config)
        end
      end
      vm.vifs = vifs
      vm.reload
